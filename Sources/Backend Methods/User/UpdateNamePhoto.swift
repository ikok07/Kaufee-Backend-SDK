...
extension Backend {
    public func updateNamePhoto(name: String, image: UIImage?, authToken: String, callback: (Result<ProfileDataResponse, BackendError<String>>)  async -> Void) async {
        ...
        let request: Result<ProfileDataResponse, NetworkError>? = await Request.formData(httpMethod: "PATCH" ,url: "\(config.baseUrl)/\(config.language)/api/v1/user/me", json: name.data(using: .utf8)!, image: image, authToken: authToken)
